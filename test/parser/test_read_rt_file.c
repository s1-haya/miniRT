...
{
	bool	result = true;
	t_minirt_list	*list = read_rt_file("./ft_file/success/rt.rt.rt", &result);
	TEST_ASSERT_TRUE(result);
	TEST_ASSERT_NOT_NULL(list);
	if (list == NULL)
		return ;
...
{
	bool	result = true;
	t_minirt_list	*list = read_rt_file("./ft_file/error/t.rtxt_rt", &result);
...
{
	bool	result = true;
	t_minirt_list	*list = read_rt_file("./ft_file/error/text.txt", &result);
	TEST_ASSERT_FALSE(result);
	TEST_ASSERT_NULL(list);
	if (list == NULL)
		return ;
